...
namespace System.Web.Mvc
{
    public class Select2Option : IOptionBuilder
    {
        ...
        public Select2Option TemplateResultHanderBarTemplate(HtmlHelper helper, string template)
        {
            var handlebars = helper.CreateHandlebarsTemplate(template);
            return TemplateResult(handlebars);
        }

        public Select2Option TemplateResultHanderBarTemplate(HtmlHelper helper, Func<object, HelperResult> template)
        {
            var handlebars = helper.CreateHandlebarsTemplate(template);
            return TemplateResult(handlebars);
        }

        public Select2Option TemplateResultHanderBarTemplateInlineHelper(HtmlHelper helper, HelperResult template)
        {
            var handlebars = helper.CreateHandlebarsTemplateInlineHelper(template);
            return TemplateResult(handlebars);
        }

        public Select2Option TemplateResultHanderBarTemplate(HtmlHelper helper, string id, string template)
        {
            var handlebars = helper.CreateHandlebarsTemplate(id, template);
            return TemplateResult(handlebars);
        }

        public Select2Option TemplateResultHanderBarTemplate(HtmlHelper helper, string id, Func<object, HelperResult> template)
        {
            var handlebars = helper.CreateHandlebarsTemplate(id, template);
            return TemplateResult(handlebars);
        }

        public Select2Option TemplateResultHanderBarTemplateInlineHelper(HtmlHelper helper, string id, HelperResult template)
        {
